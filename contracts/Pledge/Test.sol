...
contract ExposedRelayPledge is RelayPledge {
    ...
    function _messageIsEarlier(Pledge.Request memory withheld, Pledge.Request memory relayed) public view returns (bool) {
        return messageIsEarlier(withheld, relayed);
    }
    function _compare(bytes memory a, bytes memory b) public view returns (int16) {
        return compare(a, b);
    }
}