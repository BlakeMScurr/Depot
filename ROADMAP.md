...
The end result for the user will probably end up resembling something like an SQL table. The indexer defines schema, and queries sent to the server are operate on elements of that schema. Implementing a minimal SQL language could help in generalising the database - in particular, combining expressions, and being able to check the validity of composite statements on chain enables a great deal of functionality.